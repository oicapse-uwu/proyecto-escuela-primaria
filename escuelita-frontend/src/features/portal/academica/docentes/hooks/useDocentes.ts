...
export const useDocentes = () => {
    ...
    const fetchDocentes = useCallback(async () => {
        ...
        try {
            ...
            // ðŸ”’ FILTRAR POR SEDE DEL USUARIO ACTUAL
    ...
    const fetchUsuarios = useCallback(async () => {
        try {
            ...
            setUsuarios([]);
            try { toast.error('No se pudieron cargar los usuarios disponibles para asignaciÃ³n'); } catch {
                // Ignorar error de toast
            }
        }

        // No mock here â€” rely on API or show empty state. Errors are logged above.
    }, []);
