...
public class ModuloAccesoAspect {
    ...
    public Object validarAcceso(ProceedingJoinPoint joinPoint, RequireModulo requireModulo) 
        ...
        // Si no se puede obtener el ID del usuario pero está autenticado, permitir por ahora
        // (La seguridad es validada por Spring Security y TenantContext ya está establecido)
        if (idUsuario == null) {
            // Verificar que al menos hay autenticación
            Authentication auth = SecurityContextHolder.getContext().getAuthentication();
            if (auth != null && auth.isAuthenticated() && TenantContext.hasSedeId()) {
                System.out.println("⚠️  No se pudo extraer ID de usuario, pero hay sesión autenticada. Permitiendo acceso.");
                return joinPoint.proceed();
            }
