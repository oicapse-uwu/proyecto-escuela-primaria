...
const UsuariosPage: React.FC = () => {
    ...
    const handleSubmit = async (payload: UsuarioPortalDTO) => {
        try {
            if (usuarioEditar?.idUsuario) {
                ...
                // Si el usuario editado es el usuario logueado, actualizar el cache del TopBar
                const currentUser = escuelaAuthService.getCurrentUser();
                if (currentUser && currentUser.idUsuario === usuarioEditar.idUsuario) {
                    escuelaAuthService.updateCurrentUser({
    ...
    return (
        <div className="p-3 sm:p-4 lg:px-6 lg:py-10 space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pt-2">
                ...
                <button
                    ...
                    className="bg-gradient-to-r from-escuela to-escuela-light text-white px-6 py-2.5 rounded-lg font-semibold hover:shadow-lg transition-all flex items-center gap-2 hover:from-escuela-dark hover:to-escuela whitespace-nowrap self-start sm:self-auto"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
                <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm flex flex-col h-[130px]">
                    <div className="flex items-center justify-between">
                        <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">Usuarios activos</p>
                        <Users className="w-4 h-4 text-escuela" />
                    </div>
                    <p className="text-3xl font-bold text-gray-800 mt-2 leading-none">{usuarios.length}</p>
                    <p className="text-xs text-gray-500 mt-auto">Usuarios registrados en esta sede</p>
                </div>
                <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm flex flex-col h-[130px]">
                    <div className="flex items-center justify-between">
                        <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">Roles en uso</p>
                        <Shield className="w-4 h-4 text-escuela" />
                    </div>
                    <p className="text-3xl font-bold text-gray-800 mt-2 leading-none">{totalRoles}</p>
                    <p className="text-xs text-gray-500 mt-auto">Cantidad de roles asignados</p>
                </div>
                <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm flex flex-col h-[130px]">
                    <div className="flex items-center justify-between">
                        <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">Sede</p>
                        <MapPin className="w-4 h-4 text-escuela" />
                    </div>
                    <p className="text-base font-semibold text-gray-800 mt-2 line-clamp-2">{sedeActualNombre}</p>
                    <p className="text-xs text-gray-500 mt-auto">Contexto actual de trabajo</p>
                </div>
                <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm flex flex-col h-[130px]">
                    <div className="flex items-center justify-between">
                    ...
                    <p className="text-base font-semibold text-gray-800 mt-2 line-clamp-2">{institucionActualNombre}</p>
                    <p className="text-xs text-gray-500 mt-auto">Entidad principal asociada</p>
                </div>
            </div>
            ...
            <div className="bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm">
                                ...
                                <tr key={item.idUsuario} className="hover:bg-gray-50 transition-colors">
                                    <td className="px-4 py-3 text-sm text-gray-700">
                                        <div className="flex items-center gap-2">
                                            <div className="w-7 h-7 rounded-full bg-escuela/10 text-escuela text-xs font-semibold flex items-center justify-center">
                                    ...
                                    <td className="px-4 py-3 text-sm text-gray-700">
                                        <div className="flex items-center justify-center gap-3">
                                            <button
                                                className="text-escuela hover:text-escuela-dark transition-colors"
                ...
                <div className="md:hidden p-3 space-y-2">
                    {usuariosPaginados.map((item) => (
                        <div key={item.idUsuario} className="border rounded-lg p-3 shadow-sm bg-white hover:shadow-md transition-shadow">
                            ...
                            <div className="mt-2 flex items-center gap-2">
                                <span className="text-[11px] px-2 py-0.5 rounded-full bg-escuela/10 text-escuela uppercase font-medium">{item.idRol?.nombre ?? '-'}</span>
                            </div>
                            <div className="mt-2 flex items-center gap-2">
                                <button
                                    className="text-escuela hover:text-escuela-dark transition-colors"
            ...
            <ModalUsuario isOpen={showForm} onClose={() => setShowForm(false)} title={usuarioEditar ? 'Editar Usuario' : 'Nuevo Usuario'}>
                <UsuarioForm
                    sedes={sedes}
                    roles={roles}
