...
const EspecialidadesPage: React.FC = () => {
    ...
    const handleEliminar = async (id: number) => {
        if (window.confirm('¿Eliminar especialidad?')) {
            try { await eliminarEspecialidadById(id); toast.success('Eliminada'); } catch (error) { toast.error('Error al eliminar'); }
        }
    };

    const handleSubmit = async (data: EspecialidadDTO) => {
        try {
            if (especialidadEditar) { await modificarEspecialidad({ ...data, idEspecialidad: especialidadEditar.idEspecialidad }); toast.success('Actualizada'); }
            else { await guardarEspecialidad(data); toast.success('Creada'); }
    ...
    return (
        <div className="p-4 lg:p-6 max-w-7xl mx-auto">
            ...
            <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => { setEspecialidadEditar(null); setIsModalOpen(true); }} className="bg-primary text-white px-4 py-2 rounded-lg flex items-center"><Plus className="w-5 h-5 mr-2" /> Nueva Especialidad</button>
            </div>
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-4 relative">
                ...
                <input type="text" placeholder="Buscar..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent" />
            </div>
            ...
            {loading ? <div className="flex justify-center py-12"><div className="animate-spin rounded-full h-10 w-10 border-4 border-primary border-t-transparent"></div></div>
                : paginadas.length === 0 ? <div className="text-center py-12 bg-white rounded-xl shadow-sm"><p className="text-gray-500">No hay datos</p></div>
                    : (
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {paginadas.map(esp => (
                                        <tr key={esp.idEspecialidad} className="hover:bg-gray-50">
                                            <td className="px-6 py-4 font-medium">{esp.nombreEspecialidad}</td>
