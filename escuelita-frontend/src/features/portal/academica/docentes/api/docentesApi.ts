...
export const docentesApi = {
    ...
    getAvailableUsers: async (): Promise<Usuario[]> => {
        // Enforce sede scoping: each sesiÃ³n sÃ³lo puede ver usuarios de su propia sede.
        const sedeId = escuelaAuthService?.getSedeId?.();
        let noSede = false;
        if (!sedeId) {
            console.warn('[docentesApi] getAvailableUsers: usuario sin sede en sesiÃ³n â€” intentaremos obtener usuarios globales (no filtraremos por sede)');
            noSede = true;
        }
        ...
        try {
            if (!noSede) {
                ...
                console.debug('[docentesApi] no sede available â€” skipping /usuarios/sede and trying global endpoints');
            }
        } catch (errSede) {
