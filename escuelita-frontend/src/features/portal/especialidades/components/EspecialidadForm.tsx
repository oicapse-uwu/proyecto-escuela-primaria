        ...
        <Modal isOpen={isOpen} onClose={onClose} title={especialidad ? 'Editar Especialidad' : 'Nueva Especialidad'} size="md">
            <form onSubmit={handleSubmit} className="p-6">
                ...
                <div className="flex justify-end space-x-3 mt-6 pt-4 border-t border-gray-100">
                    ...
                    <button type="submit" disabled={isLoading} className="px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary-dark">{isLoading ? 'Guardando...' : especialidad ? 'Actualizar' : 'Guardar'}</button>
                </div>
            </form>
        </Modal>
    );
